...
$query_works = "SELECT user_email, projectTitle, author, imagePath, videoPath, textDescription, projectCategories, projectTags1, projectTags2, projectTags3, toolsUsed1, toolsUsed2, toolsUsed3, adultContent FROM works";
    ...
    <div class="container mt-5" id="card-container">
        <div class="row row-cols-1 row-cols-md-3 g-4"> <!-- Bootstrap grid for better layout -->
            ...
            while ($row = mysqli_fetch_assoc($works_result)) { 
                ...
                $video_url = "/lpu-eportfolio" . htmlspecialchars($row['videoPath']); // Assuming videoPath is also stored
            ?>
                <div class="col">
                    ...
                    data-video="<?php echo !empty($row['videoPath']) ? $video_url : ''; ?>"
    ...
    <script>
        ...
        cardLinks.forEach(link => {
            link.addEventListener('click', function(event) {
                ...
                const videoPath = this.getAttribute('data-video');  // New attribute for video path
                ...
                document.getElementById('author_email').value = this.getAttribute('data-user-email');

                // Remove any previous video element (if exists)
                const existingVideo = modal.querySelector('video');
                if (existingVideo) {
                    existingVideo.remove();
                }

                // Determine whether to show an image or a video
                if (videoPath && videoPath.trim() !== "") {
                    // If there's a video, display the video
                    modalImage.style.display = 'none';  // Hide the image
                    let videoElement = document.createElement('video');
                    videoElement.setAttribute('controls', 'true');
                    videoElement.setAttribute('width', '100%');
                    videoElement.setAttribute('height', '100%');
                    videoElement.innerHTML = `<source src="${videoPath}" type="video/mp4">Your browser does not support the video tag.`;
                    modalImage.parentElement.appendChild(videoElement);  // Add video to modal
                } else {
                    // If no video, display the image
                    modalImage.style.display = 'block';  // Show the image
                    modalImage.src = image;  // Set image source
                }
